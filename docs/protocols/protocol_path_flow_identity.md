...
- the same lazily resolved full protocol-path text is now reused by flow-metadata CSV export, including Smart Export `flows_manifest.csv` and the Qt/Tauri `Flow -> Export All Flows Info to CSV...` action, with CSV export normalizing the path into a single-cell single-line compact value such as `EthernetII->IPv4->TCP`;

...
| PPPoE Session + PPP IPv4/IPv6 | Partial | Partial | Not supported | Supported | Partial | Partial | Partial | Supported | First PPPoE step supports only PPPoE Session (`0x8864`) data frames with `code = 0x00` and exact payload-length match, continuing through PPP IPv4 (`0x0021`) and PPP IPv6 (`0x0057`) into normal IPv4/IPv6/TCP/UDP parsing. Selected-packet layered Summary now preserves PPPoE Session and PPP protocol layers. PPPoE Discovery, PPP control protocols, unknown PPP protocols, and malformed/length-mismatch cases remain conservative/unrecognized. |

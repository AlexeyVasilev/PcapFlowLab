...
| PPPoE Session + PPP IPv4/IPv6 + basic PPPoE Discovery | Partial | Partial | Not supported | Supported | Partial | Partial | Partial | Supported | Current PPPoE support covers PPPoE Session (`0x8864`) data frames with `code = 0x00` and exact payload-length match, continuing through PPP IPv4 (`0x0021`) and PPP IPv6 (`0x0057`) into normal IPv4/IPv6/TCP/UDP parsing. Selected-packet layered Summary now preserves PPPoE Session and PPP protocol layers, and basic PPPoE Discovery (`0x8863`) header/tag presentation for PADI/PADO/PADR/PADS/PADT. Discovery packets remain no-flow. PPP control protocols, unknown PPP protocols, and malformed/length-mismatch cases remain conservative/unrecognized. |
...
- `pppoe`
  - deterministic PPPoE Session, PPP control, PPPoE Discovery, VLAN/QinQ-before-PPPoE, unknown PPP protocol, and malformed/truncated/length-mismatch fixtures. The current parser supports normal PPPoE Session IPv4/IPv6 data continuation plus basic PPPoE Discovery header/tag presentation for no-flow Discovery packets, while PPP control and malformed/length-mismatch cases remain conservative.
- `quic`

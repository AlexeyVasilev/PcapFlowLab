...
| mDNS | Supported | Not supported | Supported | Supported | Supported | Partial | Partial | Supported | Detection still depends on UDP/5353 and multicast destination checks. Flow-level Service uses a bounded best-effort Question/PTR-owner/RR-owner hint, selected-packet Summary is structured, and user-facing compact detected text is `mDNS`. |

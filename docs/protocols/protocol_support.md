...
| VXLAN / Geneve UDP overlays | Partial | Partial | Not supported | Partial | Partial | Partial | Partial | Supported | Current overlay-inner-tuple support recognizes VXLAN over UDP/4789 and Geneve over UDP/6081 during open/import when a valid bounded overlay header leads to inner Ethernet plus inner IPv4/IPv6 plus TCP/UDP. The effective flow tuple becomes the inner transport tuple, including existing inner VLAN continuation where valid. VXLAN and Geneve selected-packet Summary / Protocol details are implemented, with sequential inner Ethernet/VLAN/IP/transport layers for valid overlays and lenient warning-only presentation for malformed/truncated overlay-like UDP payloads on their standard ports. For Geneve, strict flow extraction still requires version `0`, 4-byte-unit option bounds that fit the captured payload, and Ethernet Protocol Type (`0x6558`); selected-packet details may still show best-effort inner continuation for unsupported-version Geneve when those other bounds remain valid, while unsupported protocol types are reported distinctly and do not continue into inner Ethernet decoding. Tunnel namespace metadata such as VNI is not yet part of flow identity, so identical inner tuples from different VNIs can still merge. |

...
| VXLAN / Geneve / GTP-U UDP overlays | Partial | Partial | Not supported | Partial | Partial | Partial | Partial | Supported | Current overlay-inner-tuple support recognizes VXLAN over UDP/4789, Geneve over UDP/6081, and GTPv1-U T-PDU over UDP/2152 during open/import. VXLAN and Geneve require a valid bounded overlay header that leads to inner Ethernet plus inner IPv4/IPv6 plus TCP/UDP, including existing inner VLAN continuation where valid. GTP-U supports the basic direct-inner-IP user-plane path, where the inner payload starts directly with IPv4 or IPv6 plus TCP/UDP. The effective flow tuple becomes the inner transport tuple. VXLAN and Geneve selected-packet Summary / Protocol details are implemented, with sequential inner Ethernet/VLAN/IP/transport layers for valid overlays and lenient warning-only presentation for malformed/truncated overlay-like UDP payloads on their standard ports. For Geneve, strict flow extraction still requires version `0`, 4-byte-unit option bounds that fit the captured payload, and Ethernet Protocol Type (`0x6558`); selected-packet details may still show best-effort inner continuation for unsupported-version Geneve when those other bounds remain valid, while unsupported protocol types are reported distinctly and do not continue into inner Ethernet decoding. For GTP-U, strict flow extraction requires version `1`, PT set, message type T-PDU `0xff`, bounded optional fields when E/S/PN are set, and a bounded extension-header chain when `E=1`. Tunnel namespace metadata such as VNI or TEID is not yet part of flow identity, so identical inner tuples from different overlay namespaces can still merge. |

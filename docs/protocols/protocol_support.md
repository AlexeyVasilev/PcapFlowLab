...
| VXLAN / Geneve UDP overlays | Partial | Partial | Not supported | Partial | Not supported | Partial | Partial | Supported | Current overlay-inner-tuple support recognizes VXLAN over UDP/4789 and Geneve over UDP/6081 during open/import when a valid bounded overlay header leads to inner Ethernet plus inner IPv4/IPv6 plus TCP/UDP. The effective flow tuple becomes the inner transport tuple, including existing inner VLAN continuation where valid. VXLAN selected-packet Summary / Protocol details are implemented. Geneve selected-packet presentation remains follow-up work in this branch. For Geneve, version must be 0, option length is interpreted in 4-byte units, bounded options are skipped safely, and Protocol Type must be Ethernet (`0x6558`). Tunnel namespace metadata such as VNI is not yet part of flow identity, so identical inner tuples from different VNIs can still merge. |
...
- `geneve`
  - deterministic Geneve fixtures including inner IPv4/IPv6 TCP/UDP continuation, bounded option skipping, VNI boundary values, inner VLAN composition, malformed/truncated headers, wrong-port negatives, and conservative unsupported-protocol cases. Current committed behavior covers flow tuple extraction only; selected-packet Geneve presentation remains follow-up work.

...
- QUIC packet and Stream details now append narrow TLS `ClientHello` / `ServerHello` fields when the current QUIC CRYPTO bytes are directly parseable in the selected-flow path.

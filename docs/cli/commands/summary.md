...
If the stored source capture is unavailable, summary must still work. An
...
`validate_selected_packet_checksums` is accepted as part of the shared CLI
settings schema, but it does not change current `summary` output.

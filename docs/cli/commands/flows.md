...
The current production `summary` command also supports:

```text
pcap-flow-lab summary capture.pcap --out-flows-list flows.csv
```

It reuses the same shared exporter with:
...
The `flows` command still owns filtered, sorted, or limited flow-list export.

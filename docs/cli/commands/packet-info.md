...
- optional Flow Context for flow-scoped selection
...
For flow-scoped selection, Flow Context appears near the beginning of the
report, before protocol Summary.
...
### Capture-Global Packet Selection

`--packet-in-file` is capture-global and packet-oriented.

The current implementation must not be documented as requiring or presenting
canonical flow ownership for that selector.

In particular, global packet selection must not be documented as reporting:
...
unless the current implementation actually renders those fields.

The current production contract keeps flow ownership/context explicit only for:
...
The packet bytes are not stored inside the index itself.
...
- Flow Context (flow-scoped selection only)
- Packet
- Summary
...
Current capture-global lookup uses dedicated source-backed global packet lookup
and the capture-level sparse packet locator persisted by the current index
format.
...
For PCAPNG, lookup may still require additional structural walking before the
anchor to restore section and interface context. The documentation therefore
must not promise the same strict bounded lookup cost for PCAPNG as for classic
PCAP.
...
- heading
- Flow Context for flow-scoped selection
- Packet
- Summary
...
| Flow Context metadata | flow-scoped only | flow-scoped only | flow-scoped only |

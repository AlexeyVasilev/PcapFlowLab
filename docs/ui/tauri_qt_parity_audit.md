...
| Open capture workflow | Native open dialog, Fast/Deep open modes, explicit open progress, session-apply overlay, and controller-owned status/error text. | Native file dialog plus typed-path fallback, open mode selector, open-state badge, inline status text, and source warning banner. | Tauri has the workflow, but progress UX is simpler and top-shell wording differs from Qt. | Medium | Tighten copy and progress presentation to match Qt more closely without changing backend behavior. |
| Open index workflow | Native open dialog and index-backed session shell with expected-source state. | Native open dialog and typed-path fallback share the same index workflow. | Workflow exists, but index-shell wording/polish remains thinner than Qt. | Medium | Small parity pass on index shell text, path messaging, and unavailable-source presentation. |
...
| Packet list | Qt packet list is bounded, supports `Load more`, shows direction chips, truncation/fragment cues, and a dedicated marker column only when needed. | Tauri packet list is bounded and supports pagination, but does not yet mirror the same visual richness or `Load more` semantics. | Presentation and paging UX differ; Tauri packet list remains one of the more visible parity gaps. | High | First packet-list parity pass: compact row styling, marker treatment, and better large-flow navigation affordances. |
...
1. Whether the current Tauri packet paging UX should stay page-based or move closer to Qt `Load more` semantics for parity.

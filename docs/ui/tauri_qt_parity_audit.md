...
| Smart Export | Qt has a richer dialog plus explicit progress/cancel UI during long-running export, including async/progress/cancel for single-file Smart Export. | Tauri mirrors scope/base/output options and folder mode, but still uses one-shot Smart Export command paths with only busy/status-level feedback. | Missing richer Qt progress/cancel UX parity and some dialog-level copy/layout fidelity. This affects all Smart Export targets, including flow-based export and `Unrecognized packets`. | Medium | Small UI polish pass: align labels/help text first, then add async Smart Export progress/cancel parity. |
...
| Flows table | Qt uses a dense list-based table with checked-flow boxes, sort buttons, filter, filtered-flow count status, Wireshark filter row, compact `Endpoint A` / `Endpoint B` columns, and horizontal scrolling for wide endpoint-heavy layouts. | Tauri has filtering, sorting, checked-flow state, filtered-flow count status, Wireshark filter row toggle, frontend virtualization/windowing, and matching endpoint-style columns with the same address/port formatting rules. | Tauri is closer functionally; the remaining gap is mostly row styling and compactness polish rather than column semantics. | Medium | UI polish pass focused on row density, spacing, and top-of-flows controls. |
...
| Packet details | Qt packet/stream details pane is richer: warnings block extraction, dynamic header for stream items, better text panes, tighter tab behavior, and selectable/copyable Summary text in the Qt inspector. | Tauri now supports `Summary / Bytes`, a shared packet-byte selector backed by stable ids and selected-view-only materialization, checksum setting, and the same shared layered packet-summary tree as Qt for the Summary tab, with lightweight native-select indentation derived from shared descriptor depth metadata. | The main gap is now inspector polish and deeper structured presentation, not packet-byte capability or the lack of a shared Summary structure. Qt currently retains the better copy/select story in the inspector. | Medium | Keep packet-details follow-ups focused on expanding structured decoded-layer presentation carefully rather than broad workflow changes. |
...
| Stream item details | Qt has a dedicated stream-item detail presentation through the packet-details pane, with contextual headers, structured summary text, item-data bytes, and source-packet summaries. | Tauri now shows a compact header block plus `Summary / Item Data` tabs driven by shared stream-item DTO fields and bounded fallback text. | The major workflow gap is closed; remaining differences are mostly formatting polish and the lack of stream-to-packet navigation. | Medium | Keep future work narrow: structured presentation polish and optional stream-to-packet navigation only. |
| Statistics overview | Qt uses summary cards plus transport/family tables that stay visible while five optional sections load lazily per capture. | Tauri now mirrors the same structure: overview cards plus transport/family always visible, with five optional collapsible sections loaded on first eligible expansion and reused for the current capture. | Main remaining gap is visual compactness and percentage-format polish rather than section structure or lazy-loading behavior. | Medium | Statistics polish pass focused on percentages, compactness, and drill-down affordances. |
...
- Qt single-file Smart Export now has async/progress/cancel, while Tauri Smart Export still lacks detailed packet-level progress and cancellation for `Current flow`, `Selected flows`, `Unselected flows`, `All flows`, `Matching current filter`, `Not matching current filter`, and `Unrecognized packets`

...
| Statistics overview | Qt uses summary cards plus transport/family tables that stay visible while six optional sections load lazily per capture. | Tauri now mirrors the same structure: overview cards plus transport/family always visible, with six optional collapsible sections loaded on first eligible expansion and reused for the current capture. | Main remaining gap is visual compactness and percentage-format polish rather than section structure or lazy-loading behavior. | Medium | Statistics polish pass focused on percentages, compactness, and drill-down affordances. |

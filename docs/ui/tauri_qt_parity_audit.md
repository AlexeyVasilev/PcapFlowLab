...
| Analysis details | Qt has the broader selected-flow analysis workspace including summary, protocol panel, derived metrics, burst/idle, histograms, sequence preview, and rate graph. | Tauri now covers most read-only sections with a Qt-closer right-side order and more Qt-like compact presentation: overview, protocol panel, derived metrics plus burst/idle, directional, TCP controls, histograms, sequence preview, and CSV export. | Biggest missing piece is still the Qt rate graph, plus some remaining presentation polish. | Medium | Keep the current frontend-only order/layout pass small; revisit rate-graph parity only if it remains worthwhile. |

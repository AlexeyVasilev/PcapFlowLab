...
| protocol hint groups | Qt derives grouped table from `protocolHintDistribution` in `MainController`, backed by `CaptureProtocolSummary` hint buckets | dedicated `FrontendProtocolHintStatisticsDto` via `get_protocol_hint_statistics()` | Tauri renders `Detected Protocol Hints` from the dedicated lazy section request instead of the eager overview DTO | percentages and richer grouping presentation still remain Qt-owned; drill-down remains filter-based in Tauri | frontend-neutral DTO for row facts, frontend formatting for presentation density | Improved |

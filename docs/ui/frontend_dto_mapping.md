...
- A narrow shared `export_selected_flows(path, flow_indices)` workflow over `CaptureSession::export_flows_to_pcap(...)` -> Tauri `Flow -> Export Selected Flows`
...
- `Save Index`, `Export Current Flow`, `Export Selected Flows`, and attach-source are now wired, so the next value is in the broader export surface.

...
- Stream:
  ...
  - Tauri now models selected stream item state and richer stream-item details with shared `Summary / Item Data / Protocol` rendering backed by the same selected-item DTO contract, but still does not navigate to source packets.
...
| stream item details header / summary / item-data / protocol text | Qt `PacketDetailsViewModel` stream-item header fields plus `summaryText`, `streamItemData*`, `payloadTabTitle`, `protocolText` | `FrontendStreamItemDto` now carries display-oriented header/summary/protocol fields plus nested `stream_item_data` metadata and one selected formatted text result | Tauri now renders a compact header block plus `Summary / Item Data / Protocol` tabs from the shared stream item DTO | old preview-oriented payload fields remain for compatibility, but `stream_item_data` is now the semantic authority | frontend-neutral DTO for selected-item details, shared backend ownership in C++ | Improved |

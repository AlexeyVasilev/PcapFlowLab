...
- Stream:
  ...
  - Tauri now models selected stream item state and richer stream-item details with shared `Summary / Item Data` rendering backed by the same selected-item DTO contract, while retained protocol text stays internal for later audit and Tauri still does not navigate to source packets.
...
| retained protocol details text | `PacketDetailsViewModel.protocolText` | `FrontendPacketDetailsDto.protocol_details_text` | `PacketDetailsDto.protocol_details_text` | aligned; visible Protocol tabs were removed, but the DTO field is intentionally retained for later audit | frontend-neutral DTO | High |
...
| stream item details header / summary / item-data / retained protocol text | Qt `PacketDetailsViewModel` stream-item header fields plus `summaryText`, `streamItemData*`, `payloadTabTitle`, `protocolText` | `FrontendStreamItemDto` now carries display-oriented header/summary/protocol fields plus nested `stream_item_data` metadata and one selected formatted text result | Tauri now renders a compact header block plus `Summary / Item Data`; retained protocol text stays in the shared DTO for later audit | old preview-oriented payload fields remain for compatibility, but `stream_item_data` is now the semantic authority | frontend-neutral DTO for selected-item details, shared backend ownership in C++ | Improved |

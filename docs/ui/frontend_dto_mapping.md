...
- Wireshark filter generation:
  ...
  - Tauri rebuilds a conservative filter in `web/main.js`.
- Packet inspector:
...
| `flow_display_number` if present | Qt displays 1-based number from `flow_index + 1` in `FlowTable.qml` | none explicit | Tauri currently renders raw `flow_index` in table and messages | naming/display mismatch between Qt and Tauri | frontend rendering over stable `flow_index` | Medium |
| address family | `FlowRow.family`; `FlowListModel.FamilyRole` formatted to `IPv4`/`IPv6` | `FrontendFlowDto.family` enum | `FlowDto.family` string `"ipv4"/"ipv6"`; not displayed in current Tauri table | Tauri DTO keeps it but current UI omits the column | frontend-neutral DTO already sufficient | Medium |
...
| protocol hint | `FlowRow.protocol_hint`; `FlowListModel.ProtocolHintRole` formats display text | `FrontendFlowDto.protocol_hint` raw-ish text | `FlowDto.protocol_hint`; shown directly in Tauri table/filter | Qt applies display formatting; Tauri shows adapter text directly | frontend-neutral DTO + later naming/display decision | Medium |
...
| fragmentation indicator/count | `has_fragmented_packets`, `fragmented_packet_count`; `Frag` column in Qt | `FrontendFlowDto.has_fragmented_packets`, `fragmented_packet_count` | `FlowDto.has_fragmented_packets`, `fragmented_packet_count`; not shown as dedicated column in Tauri | Tauri currently hides part of contract | frontend-neutral DTO already sufficient | Medium |
...
| Wireshark display filter | Qt `MainController.selectedFlowWiresharkFilter()` via shared helper over flow model | no field in `FrontendFlowDto`; no adapter API returning filter string | Tauri assembles conservative filter in `main.js` from `family`, `address`, `port`, `protocol_text` | duplicated generation logic, no shared owner yet | candidate frontend-neutral DTO field or shared helper | High |
...
| IP fragmentation marker | `PacketRow.is_ip_fragmented`; `IsIpFragmentedRole` | `FrontendPacketDto.is_ip_fragmented` | `PacketDto.is_ip_fragmented`; not shown in current Tauri table | Tauri UI gap, data exists | frontend-neutral DTO already sufficient | Medium |
| suspected retransmission marker | `PacketRow.suspected_tcp_retransmission`; `SuspectedTcpRetransmissionRole`; `hasVisibleMarkers` in Qt model | `FrontendPacketDto.suspected_tcp_retransmission`; adapter derives via `suspected_tcp_retransmission_packet_indices(...)` | `PacketDto.suspected_tcp_retransmission`; not shown in current Tauri table | data exists, UI diverges | frontend-neutral DTO already sufficient | Medium |

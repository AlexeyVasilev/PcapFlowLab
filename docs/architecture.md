...
- `core/io`: classic PCAP file access, random-access byte reading, lazy packet-data retrieval by file offset, and classic PCAP writing.
...
- Current import scope: the import path supports classic PCAP only, and the current decode path supports Ethernet II with up to two VLAN tags plus IPv4/IPv6 with TCP/UDP. Richer layered decode will come later.

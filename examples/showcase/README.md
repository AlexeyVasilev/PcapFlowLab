...
11. Open `APP-TLS-SEGMENTED-01` to demonstrate the segmented TLS ClientHello case where the Flow immediately shows Service `edge.microsoft.com` after opening the showcase capture, even though the SNI lives in the second TCP segment. Selecting the Flow still demonstrates bounded TLS reconstruction and structured inspection, but selection is no longer required to obtain Service/SNI for this exact two-segment case.

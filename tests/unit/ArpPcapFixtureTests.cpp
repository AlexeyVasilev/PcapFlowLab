...
void run_arp_pcap_fixture_tests() {
...

    {
        ...
        PFL_EXPECT(session.find_packet(0U).has_value());
        PFL_EXPECT(session.find_packet(1U).has_value());
        PFL_EXPECT(session.read_packet_protocol_details_text(*session.find_packet(0U)).find("Opcode: request (1)") != std::string::npos);
        PFL_EXPECT(session.read_packet_protocol_details_text(*session.find_packet(1U)).find("Opcode: reply (2)") != std::string::npos);
    }

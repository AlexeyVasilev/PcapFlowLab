...
std::vector<std::uint8_t> make_mqtt_connect_payload() {
    ...
    payload.push_back(0x1AU);
    append_be16(payload, 4U);
    payload.insert(payload.end(), {'M', 'Q', 'T', 'T'});
    payload.push_back(0x04U);
    payload.push_back(0x02U);
    append_be16(payload, 60U);
    append_be16(payload, 14U);
    payload.insert(payload.end(), {'p', 'f', 'l', '-', 'm', 'q', 't', 't', '-', 'c', 'l', 'i', 'e', 'n'});
    return payload;
}

std::vector<std::uint8_t> make_amqp091_header_payload() {
    return std::vector<std::uint8_t> {'A', 'M', 'Q', 'P', 0x00U, 0x00U, 0x09U, 0x01U};
}

std::vector<std::uint8_t> make_ntp_client_payload() {
    std::vector<std::uint8_t> payload(48U, 0U);
    payload[0] = static_cast<std::uint8_t>((4U << 3U) | 3U);
    payload[1] = 2U;
    payload[2] = 6U;
    payload[3] = 0xECU;
    payload[40] = 0xE7U;
...
void run_protocol_summary_tests() {
    {
        ...
        expect_protocol_stats(summary.tcp, ProtocolStats {1, 1, static_cast<std::uint64_t>(tcp_packet.size()), static_cast<std::uint64_t>(tcp_packet.size())});
...

    {
        ...
        const auto mqtt_packet = make_ethernet_ipv4_tcp_packet_with_bytes_payload(
            ipv4(10, 10, 0, 9), ipv4(10, 10, 0, 10), 57000, 1883, make_mqtt_connect_payload(), 0x18
        );
        const auto amqp_packet = make_ethernet_ipv4_tcp_packet_with_bytes_payload(
            ipv4(10, 10, 0, 11), ipv4(10, 10, 0, 12), 58000, 5672, make_amqp091_header_payload(), 0x18
        );
        const auto ntp_packet = make_ethernet_ipv4_udp_packet_with_bytes_payload(
            ipv4(10, 10, 0, 13), ipv4(10, 10, 0, 14), 59000, 123, make_ntp_client_payload()
        );
        ...
        expect_protocol_stats(summary.hint_ssh, ProtocolStats {1, 1, static_cast<std::uint64_t>(ssh_packet.size()), static_cast<std::uint64_t>(ssh_packet.size())});
        expect_protocol_stats(summary.hint_stun, ProtocolStats {1, 1, static_cast<std::uint64_t>(stun_packet.size()), static_cast<std::uint64_t>(stun_packet.size())});
        expect_protocol_stats(summary.hint_bittorrent, ProtocolStats {1, 1, static_cast<std::uint64_t>(bittorrent_packet.size()), static_cast<std::uint64_t>(bittorrent_packet.size())});
        expect_protocol_stats(summary.hint_mqtt, ProtocolStats {1, 1, static_cast<std::uint64_t>(mqtt_packet.size()), static_cast<std::uint64_t>(mqtt_packet.size())});
        expect_protocol_stats(summary.hint_amqp, ProtocolStats {1, 1, static_cast<std::uint64_t>(amqp_packet.size()), static_cast<std::uint64_t>(amqp_packet.size())});
        expect_protocol_stats(summary.hint_ntp, ProtocolStats {1, 1, static_cast<std::uint64_t>(ntp_packet.size()), static_cast<std::uint64_t>(ntp_packet.size())});
        expect_protocol_stats(summary.hint_unknown, ProtocolStats {1, 1, static_cast<std::uint64_t>(unknown_packet.size()), static_cast<std::uint64_t>(unknown_packet.size())});
        ...
        const auto hint_flow_total = summary.hint_http.flow_count + summary.hint_tls.flow_count + summary.hint_dns.flow_count +
            ...
            summary.hint_bittorrent.flow_count + summary.hint_mqtt.flow_count + summary.hint_amqp.flow_count + summary.hint_ntp.flow_count + summary.hint_mail_protocols.flow_count + summary.hint_dhcp.flow_count + summary.hint_mdns.flow_count + summary.hint_possible_tls.flow_count + summary.hint_possible_quic.flow_count + summary.hint_unknown.flow_count;
        const auto hint_packet_total = summary.hint_http.packet_count + summary.hint_tls.packet_count + summary.hint_dns.packet_count +
            ...
            summary.hint_bittorrent.packet_count + summary.hint_mqtt.packet_count + summary.hint_amqp.packet_count + summary.hint_ntp.packet_count + summary.hint_mail_protocols.packet_count + summary.hint_dhcp.packet_count + summary.hint_mdns.packet_count + summary.hint_possible_tls.packet_count + summary.hint_possible_quic.packet_count + summary.hint_unknown.packet_count;
        const auto hint_byte_total = summary.hint_http.original_bytes + summary.hint_tls.original_bytes + summary.hint_dns.original_bytes +
            summary.hint_quic.original_bytes + summary.hint_ssh.original_bytes + summary.hint_stun.original_bytes +
            summary.hint_bittorrent.original_bytes + summary.hint_mqtt.original_bytes + summary.hint_amqp.original_bytes + summary.hint_ntp.original_bytes + summary.hint_mail_protocols.original_bytes + summary.hint_dhcp.original_bytes + summary.hint_mdns.original_bytes + summary.hint_possible_tls.original_bytes + summary.hint_possible_quic.original_bytes + summary.hint_unknown.original_bytes;
...

    {
        ...
        expect_protocol_stats(summary.hint_smtp, ProtocolStats {1, 1, static_cast<std::uint64_t>(smtp_packet.size()), static_cast<std::uint64_t>(smtp_packet.size())});
        expect_protocol_stats(summary.hint_pop3, ProtocolStats {1, 1, static_cast<std::uint64_t>(pop3_packet.size()), static_cast<std::uint64_t>(pop3_packet.size())});
        expect_protocol_stats(summary.hint_imap, ProtocolStats {1, 1, static_cast<std::uint64_t>(imap_packet.size()), static_cast<std::uint64_t>(imap_packet.size())});
        ...
        PFL_EXPECT(summary.hint_mail_protocols.original_bytes == summary.hint_smtp.original_bytes + summary.hint_pop3.original_bytes + summary.hint_imap.original_bytes);
    ...
    }
    {
        ...
        expect_protocol_stats(summary.hint_dhcp, ProtocolStats {1, 1, static_cast<std::uint64_t>(dhcp_packet.size()), static_cast<std::uint64_t>(dhcp_packet.size())});
        expect_protocol_stats(summary.hint_mdns, ProtocolStats {1, 1, static_cast<std::uint64_t>(mdns_packet.size()), static_cast<std::uint64_t>(mdns_packet.size())});
        expect_protocol_stats(summary.hint_unknown, ProtocolStats {1, 1, static_cast<std::uint64_t>(unknown_packet.size()), static_cast<std::uint64_t>(unknown_packet.size())});

        const auto hint_flow_total = summary.hint_http.flow_count + summary.hint_tls.flow_count + summary.hint_dns.flow_count +
            summary.hint_quic.flow_count + summary.hint_ssh.flow_count + summary.hint_stun.flow_count +
            summary.hint_bittorrent.flow_count + summary.hint_mqtt.flow_count + summary.hint_amqp.flow_count + summary.hint_ntp.flow_count + summary.hint_mail_protocols.flow_count + summary.hint_dhcp.flow_count +
            summary.hint_mdns.flow_count + summary.hint_unknown.flow_count;
        const auto hint_packet_total = summary.hint_http.packet_count + summary.hint_tls.packet_count + summary.hint_dns.packet_count +
            summary.hint_quic.packet_count + summary.hint_ssh.packet_count + summary.hint_stun.packet_count +
            summary.hint_bittorrent.packet_count + summary.hint_mqtt.packet_count + summary.hint_amqp.packet_count + summary.hint_ntp.packet_count + summary.hint_mail_protocols.packet_count + summary.hint_dhcp.packet_count +
            summary.hint_mdns.packet_count + summary.hint_unknown.packet_count;
        const auto hint_byte_total = summary.hint_http.original_bytes + summary.hint_tls.original_bytes + summary.hint_dns.original_bytes +
            summary.hint_quic.original_bytes + summary.hint_ssh.original_bytes + summary.hint_stun.original_bytes +
            summary.hint_bittorrent.original_bytes + summary.hint_mqtt.original_bytes + summary.hint_amqp.original_bytes + summary.hint_ntp.original_bytes + summary.hint_mail_protocols.original_bytes + summary.hint_dhcp.original_bytes +
            summary.hint_mdns.original_bytes + summary.hint_unknown.original_bytes;
...

    {
        ...
        expect_protocol_stats(summary.hint_tls, ProtocolStats {1, 1, static_cast<std::uint64_t>(confirmed_tls_packet.size()), static_cast<std::uint64_t>(confirmed_tls_packet.size())});
        expect_protocol_stats(summary.hint_quic, ProtocolStats {1, 1, static_cast<std::uint64_t>(confirmed_quic_packet.size()), static_cast<std::uint64_t>(confirmed_quic_packet.size())});
        expect_protocol_stats(summary.hint_possible_tls, ProtocolStats {1, 1, static_cast<std::uint64_t>(possible_tls_packet.size()), static_cast<std::uint64_t>(possible_tls_packet.size())});
        expect_protocol_stats(summary.hint_possible_quic, ProtocolStats {1, 1, static_cast<std::uint64_t>(possible_quic_packet.size()), static_cast<std::uint64_t>(possible_quic_packet.size())});
        expect_protocol_stats(summary.hint_unknown, ProtocolStats {1, 1, static_cast<std::uint64_t>(unknown_packet.size()), static_cast<std::uint64_t>(unknown_packet.size())});
        ...
        const auto hint_byte_total = summary.hint_http.original_bytes + summary.hint_tls.original_bytes + summary.hint_dns.original_bytes +
            summary.hint_quic.original_bytes + summary.hint_ssh.original_bytes + summary.hint_stun.original_bytes +
            summary.hint_bittorrent.original_bytes + summary.hint_mqtt.original_bytes + summary.hint_amqp.original_bytes + summary.hint_ntp.original_bytes + summary.hint_mail_protocols.original_bytes + summary.hint_dhcp.original_bytes +
            summary.hint_mdns.original_bytes + summary.hint_possible_tls.original_bytes + summary.hint_possible_quic.original_bytes + summary.hint_unknown.original_bytes;

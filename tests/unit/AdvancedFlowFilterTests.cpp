...
void run_protocol_and_candidate_scope_tests() {
...

    {
        ...
        expect_indices_equal(evaluate_matching_indices(mqtt_connections, filter), {1U, 2U});
        ...
        filter = require_compiled_filter(spec, fixture, fixture.default_settings);
        expect_indices_equal(evaluate_matching_indices(mqtt_connections, filter), {0U, 1U});
    }
}
...
void run_frontend_structured_document_tests() {
    ...
    PFL_EXPECT(empty.option_catalog.detected_protocol.size() == 19U);

...
void expect_ipv4_canonical_parsers_and_fragment_basics() {
    ...
    const auto tcp_ipv4 = parse_ipv4_packet(require_child_slice(
        tcp_root,
        tcp_ethernet.header_length,
        tcp_ethernet.declared_payload_length
    ));
...
void expect_arp_canonical_parsers_and_bounds() {
...
void run_common_direct_network_dissection_tests() {
    expect_ipv4_canonical_parsers_and_fragment_basics();
    expect_arp_canonical_parsers_and_bounds();

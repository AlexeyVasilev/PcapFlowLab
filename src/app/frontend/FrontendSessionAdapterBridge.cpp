...
std::string source_availability_json(const pfl::FrontendSourceAvailabilityDto& source) {
    ...
    out << '{'
        ...
        << "\"flow_grouping_ignores_vlan_layers\":" << bool_json(source.flow_grouping_ignores_vlan_layers) << ','
...
std::string settings_json(const pfl::FrontendSettingsDto& settings) {
    ...
    out << '{'
        ...
        << "\"ignore_vlan_layers_when_grouping_flows\":" << bool_json(settings.ignore_vlan_layers_when_grouping_flows) << ','
...
char* pfl_frontend_session_adapter_get_settings_json(PflFrontendSessionAdapterHandle* handle) {
    if (handle == nullptr) {
        return make_c_string("{\"http_use_path_as_service_hint\":false,\"use_possible_tls_quic\":false,\"ignore_vlan_layers_when_grouping_flows\":false,\"show_wireshark_filter_for_selected_flow\":true,\"validate_selected_packet_checksums\":false}");
    }
...
char* pfl_frontend_session_adapter_update_settings_json(
    ...
    const std::uint8_t ignore_vlan_layers_when_grouping_flows,
    ...
    return make_c_string(settings_json(handle->adapter.update_settings(pfl::FrontendSettingsDto {
        ...
        .ignore_vlan_layers_when_grouping_flows = ignore_vlan_layers_when_grouping_flows != 0U,

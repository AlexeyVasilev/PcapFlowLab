...
inline constexpr std::uint32_t kLinkTypeErf = 197U;
...
[[nodiscard]] constexpr std::string_view capture_link_type_name(const std::uint32_t link_type) noexcept {
    ...
    case kLinkTypeErf:
        return "Extensible Record Format";
...
[[nodiscard]] constexpr bool is_supported_classic_pcap_link_type(const std::uint32_t link_type) noexcept {
    return is_supported_capture_link_type(link_type) || link_type == kLinkTypeErf;
}
